...
import { 
  BarChart3, LineChart, Table as TableIcon, 
...
// --- 配置数据 ---
const DIMENSIONS = [
  { id: 'd1', name: '大区', category: '店铺信息' },
  { id: 'd2', name: '区域', category: '店铺信息' },
  { id: 'd3', name: '督导', category: '店铺信息' },
  { id: 'd4', name: '大类名称', category: '分类信息' },
  { id: 'd5', name: '品类经理', category: '分类信息' },
  { id: 'd6', name: '商品名称', category: '商品信息' },
  { id: 'd7', name: '全店销量ABC级别', category: '商品信息' },
  { id: 'd8', name: '供应商名称', category: '供应商信息' },
];
...
const METRICS = [
  ...
  { id: 'm5', name: '周转天数', category: '库存' },
  { id: 'm6', name: '库存金额(期末)', category: '库存' },
  { id: 'm7', name: '动销率', category: '销售', unit: '%' },
];
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className={`bg-white border-r border-slate-200 flex flex-col z-20 transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'w-20' : 'w-80'}`}>
          <div className="p-4 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">            
            {!sidebarCollapsed && (
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100 text-white">
                  <Database className="w-5 h-5" />
                </div>
                <h1 className="font-black text-lg tracking-tight">玲珑报表 <span className="text-slate-300 font-normal ml-1">LingoBI</span></h1>
              </div>
            )}
            <button 
              onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
              className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 rounded-2xl text-slate-500 transition-colors"
          ...
          <div className={`${sidebarCollapsed ? 'hidden' : 'flex-1 overflow-y-auto p-2 space-y-4 custom-scrollbar'}`}>
            ...
            {/* 维度组 */}
            <div>
              <div className="px-3 py-2 text-indigo-600 font-black text-[10px] flex items-center gap-2 uppercase tracking-widest">维度分析</div>
            ...
            {/* 指标组 */}
            <div>
              <div className="px-3 py-2 text-orange-600 font-black text-[10px] flex items-center gap-2 uppercase tracking-widest">度量指标</div>
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-slate-50/50">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-1">
              {SCENARIOS.map(s => (
                <button
                  key={s.id}
                  onClick={() => setScenario(s.id)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${scenario === s.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}
                >
                  {s.name}
                </button>
              ))}
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-3 bg-slate-100 px-4 py-1.5 rounded-full border border-slate-200">
                ...
                {scenario === 'compare' ? (
                  <div className="flex items-center gap-3">
                      <div className="px-3 py-1 bg-white rounded-lg border text-xs shadow-sm">本期: 2023-10-01 ~ 2023-10-31</div>
                      <div className="text-slate-300">vs</div>
                      <div className="px-3 py-1 bg-white rounded-lg border text-xs shadow-sm">上期: 2023-09-01 ~ 2023-09-30</div>
                  </div>
                ) : (
                  <span className="text-[11px] font-black text-slate-500 tracking-wider">2023-10-01 ~ 2023-10-31</span>
                )}
                <div className="h-3 w-px bg-slate-300 mx-1"></div>
                <Search className="w-3.5 h-3.5 text-indigo-500 cursor-pointer" />
              </div>
              <button className="p-2 hover:bg-slate-100 rounded-full text-slate-400"><Download className="w-5 h-5" /></button>
            </div>
          </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex-1 overflow-x-auto pb-2 sm:pb-0">
                    <div className="flex items-center gap-2 min-w-max">
                      <div className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 shadow-sm text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-2 mr-2">
                        <Layout className="w-3.5 h-3.5" /> 结构配置
                      </div>
                  ...
                  <>
                    <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-white">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Query Result</span>
                        <div className="px-2 py-0.5 bg-green-50 text-green-600 rounded text-[10px] font-bold border border-green-100">
                          SUCCESS: 1,284 ROWS
                        </div>
                      </div>
                      <div className="flex bg-slate-100 p-1 rounded-xl">
                        <button onClick={()=>setViewType('table')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${viewType === 'table' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><TableIcon className="w-3.5 h-3.5"/>表格</button>
                        <button onClick={()=>setViewType('chart')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${viewType === 'chart' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><BarChart3 className="w-3.5 h-3.5"/>图表</button>
                      </div>
                    </div>
                      ...
                      {viewType === 'table' ? (
                        <table className="w-full text-left border-collapse">
                          <thead className="sticky top-0 bg-slate-50 z-[5]">
                            <tr>
                              {selectedDims.map(d => (
                                <th key={d.id} className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase border-b border-slate-100">{d.name}</th>
                              ))}
                              {selectedMets.map(m => (
                                <th key={m.id} className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase border-b border-slate-100 text-right">{m.name}</th>
                              ))}
                            </tr>
                          </thead>
                        ...
                        <div className="h-full p-8 flex flex-col">
                           <div className="flex-1 min-h-[400px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={[...Array(8)].map((_, i) => ({ 
                                name: selectedDims[0]?.name ? `${DIM_DICT[selectedDims[0].name][i % 4]}` : `项目${i+1}`,
                           ...
                           <div className="mt-4 p-4 bg-slate-50 rounded-2xl text-[10px] text-slate-400 font-medium">
                             * 图表自动基于第一个维度和第一个指标进行渲染。
                           </div>
                        </div>
                      )}
                    </div>
